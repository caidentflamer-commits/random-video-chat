...
- **Creator referrals — BUILT BUT ON HOLD (2026-08-10):** Caiden's actual plan is
  **flat-fee sponsored streams** — pay creators to go live on the site, no links,
  ...
  (server warns once and no-ops until then). Payout **terms are not in code** —
  paying is manual, from the paying-now column; the recurring-share model this
  supports is "X% of live subscribers each month". the site previously had NO icon of
  any kind. Now: purple rounded square with a white O-ring, generated by
  `tools/make_icons.js` — **pure Node, no image libraries** (a PNG is zlib
  scanlines + CRCs); re-run it after design changes, same bytes every time.
  Favicon + `apple-touch-icon` + `manifest.webmanifest` (standalone, dark).
  An **"📲 Add to your phone"** chip appears on the idle screen — Android/Chrome
  only (iOS has no install API; iPhone users add manually via Share, which the
  apple-touch-icon makes look right), and only after at least one call this
  visit, so the ask comes after the product has shown something.
  **Deliberately no service worker**: offline caching is pointless for live
  video, SW staleness is a real risk, and Chrome's install prompt no longer
  requires one. ⚠ Real-device check pending: camera in **standalone** mode
  (launched from home screen) on iOS — works on modern iOS but unverified here.
